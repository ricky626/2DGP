    ...
    def draw(self):
        for i in range(0, 12):
            for j in range(0, 16):
                ...
                if(self.object[i][j] == 14): self.red_off.draw(self.objectX[i][j] + 12, self.objectY[i][j] + 28)
                if(self.object[i][j] == 15): self.yellow_off.draw(self.objectX[i][j]+ 12, self.objectY[i][j]+ 28)
